...
A Next.js app with a static bank of ~10,200 questions across 128 topics, plus a
timed mock test that follows real contest rules.
...
`src/data/math-topics.json` is the source of truth. `src/data/topics/` is a build
artifact — one file per topic plus a ~30 KB index, so the home page loads the
index instead of the whole 2.3 MB bank.

```bash
npx tsx scripts/audit.ts              # report on answer shapes, starred problems, dupes
npx tsx scripts/generate-questions.ts # add generated topics (answers computed, not authored)
npx tsx scripts/split-data.ts         # regenerate src/data/topics/ after editing the bank
```

Run `split-data.ts` after any change to `math-topics.json`, or the app will serve
stale topics.
...
Prefer a generator in `scripts/generate-questions.ts` over hand-authoring, and
compute the answer rather than typing it. Then add a case to
`src/lib/generated-questions.test.ts` that re-derives the answer from the question
text a second way. Hand-authored answers are how the original bank ended up 20%
unusable.
